...
impl AppState {
    ...
    pub fn load_topics_for_area(&mut self, area: &str) -> Result<()> {
        ...
        if area_spec_dir.exists() {
            for entry in std::fs::read_dir(&area_spec_dir)? {
                let entry = entry?;
                if entry.path().is_dir() {
                    topics.push(self.load_topic_recursive(&entry.path(), area.to_string())?);
                }
            }
        }
        self.topics = topics;
    ...
    fn load_topic_recursive(&self, path: &Path, area: String) -> Result<TopicNode> {
        ...
        if path.exists() {
            for entry in std::fs::read_dir(path)? {
                let entry = entry?;
                if entry.path().is_dir() {
                    children.push(self.load_topic_recursive(&entry.path(), area.clone())?);
                }
            }
        }
    ...
    fn load_nested_topics(&self, path: &Path, area: String) -> Result<Vec<TopicNode>> {
        ...
        if path.exists() {
            for entry in std::fs::read_dir(path)? {
                let entry = entry?;
                if entry.path().is_dir() {
                    topics.push(self.load_topic_recursive(&entry.path(), area.clone())?);
                }
            }
        }
        Ok(topics)
